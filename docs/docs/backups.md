...
During a backup job, you can avoid saving all disks of the VM. To do that is trivial: just edit the VM disk name so it contains `[NOBAK]` somewhere, eg: `data-disk` will become `[NOBAK] data-disk` or  `data-disk [NOBAK]` (with a space or not, doesn't matter).
